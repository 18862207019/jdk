...
public class ThreadPoolExecutor extends AbstractExecutorService {
    ...
    //RUNNING，表示可接受新任务，且可执行队列中的任务； -- 对应的高3位值是111。
    private static final int RUNNING    = -1 << COUNT_BITS;
    //SHUTDOWN，表示不接受新任务，但可执行队列中的任务；-- 对应的高3位值是000。
    private static final int SHUTDOWN   =  0 << COUNT_BITS;
    //STOP，表示不接受新任务，且不再执行队列中的任务，且中断正在执行的任务；    -- 对应的高3位值是001。
    private static final int STOP       =  1 << COUNT_BITS;
    // TIDYING，所有任务已经中止，且工作线程数量为0，  -- 对应的高3位值是010。
    // 最后变迁到这个状态的线程将要执行terminated()钩子方法，
    // 只会有一个线程执行这个方法；
    private static final int TIDYING    =  2 << COUNT_BITS;
    //中止状态，已经执行完terminated()钩子方法； -- 对应的高3位值是011。
    ...
    private boolean addWorker(Runnable firstTask, boolean core) {
        ...
        for (;;) {// 外层循环用于判断线程池状态
            ...
            /*rs >= SHUTDOWN ，表示当前线程处于SHUTDOWN ，STOP、TIDYING、TERMINATED状态
                    ...
                    如果workQueue == null，则说明添加的任务没有任何意义。
                     */
...
// ------上面的循环是调整线程池状态，下面是真正的添加线程--------
    ...
    public void execute(Runnable command) {
        if (command == null)
            throw new NullPointerException();
        int c = ctl.get();
        //如果线程池当前线程数小于corePoolSize，则调用addWorker创建新线程执行任务
        ...
        //如果线程池处于RUNNING状态，则尝试加入阻塞队列
        if (isRunning(c) && workQueue.offer(command)) {
            //如果加入阻塞队列成功，则尝试进行Double Check
            int recheck = ctl.get();
            //Double Check过程的主要目的是判断加入到阻塞队里中的线程是否可以被执行。
            // 如果线程池不是RUNNING状态，则调用remove()方法从阻塞队列中删除该任务，然后调用reject()方法处理任务
            if (! isRunning(recheck) && remove(command))
                reject(command);
            //确保还有线程执行
            else if (workerCountOf(recheck) == 0)
                //因为firstTask == null是为了添加一个没有任务的线程然后再从workQueue中获取任务的
                //如果线程池中的线程==0，按照道理应该该任务应该新建线程执行任务，但是由于已经该任务已经添加到了阻塞队列，
                // 那么就在线程池中新建一个空线程，然后从阻塞队列中取线程即可。
                addWorker(null, false);
        }
